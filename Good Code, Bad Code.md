...
- 널 객체 패턴을 적절히 사용하라
    - 빈 `Collection` 객체가 그냥 `Null`보다 나을 수 있음
...
- 하지만 `Enum` 사용에 주의할 점이 있음
    ...
    ⇒ `switch` 문을 사용하고 기존에 정의된 값이 아니라면 `Exception`을 `throw`하자
    
    (그리고 모든 값에 대해 `Exception`이 발생하지 않는 테스트 코드를 추가하자)
