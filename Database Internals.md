...
- 분산 시스템의 자원 공유
    - 자원을 공유할 수는 있지만, 장애 유형을 나타내는 장애 모델을 먼저 정의해야함
- 분산 컴퓨팅의 오류
    - 프로세스와 프로세스 간의 통신이 필수이기에 수많은 오류를 염두에 두어야함
    - 일반적으로는 큐를 통해 요청 (메시지) 를 관리함
    - 각 서버의 클럭이 동기화되었다는 가정을 해서는 안됨 (로컬 클럭에 의존함)
    - 상태 일관성 → 이를 보장하기 위해 다양한 방법이 있음 (12장에서)
- 분산 시스템 추상화
    - 링크 → 프로세스 간의 메시지
        - 손실 허용 링크
            - UDP 처럼 보내고 끝
...
- 리더 프로세스 (Coordinator)
    - 동기화 오버헤드를 줄이고
    - 의사 결정에 필요한 메시지 왕래 횟수를 줄임
- 한 번에 하나의 리더만 존재해야 하고 Split Brain 현상은 절대 발생하면 안됨
- 리더 프로세스의 역할
    - 브로드캐스트된 메시지의 순서 유지
    - 전역 상태 수집, 저장
    - 수신한 메시지를 각 프로세스에 전달
- 리더 선출 ↔ 분산 잠금
    - 분산 잠금: Lock 을 소유하게 될 때, 다른 프로세스는 Lock의 소유자가 누구인지 알 필요가 없음
    - 리더 선출: 모든 구성원이 리더의 존재를 알아야함
- 모든 리더 프로세스에는 장애가 발생할 수 있음 → 장애가 발생하면 새로운 리더를 선출
- Bully Algorithm
    - 순위 기반 리더 선출 알고리즘
    - 각 프로세스에 고유 순위를 부여, 가장 높은 순위의 프로세스가 리더
    - 알고리즘
        - 리더가 중단된 것을 감지한 프로세스는 자신보다 높은 순위의 프로세스들에게 Election 메시지를 전송
        - 순위가 높은 프로세스들은 Alive 메시지로 응답
        - 응답 받은 프로세스들 중 가장 순위가 높은 프로세스가 리더로 선출되고 새로운 리더 선출 사실을 전송
    - 하지만 네트워크 파티션이 발생했을 때 안정성이 보장되지 않음
    - 개선 버전 - 리더 승계 목록
        - 리더 프로세스로부터 리더 역할을 승계할 프로세스 리스트를 받고, 이를 활용하여 선출
        - 리더 선출 과정이 빨라짐
    - 개선 버전 - 후보/일반 노드 최적화
        - 노드를 후보 그룹 / 일반 그룹 으로 나눔
        - 일반 그룹의 노드가 후보 그룹의 노드의 순위만 고려햐여 다음 리더를 선출
- Invitation Algorithm
    - 높은 순위를 차지하기 위해 경장하는게 아닌, 자신의 그룹으로 초대함
    - 그룹별로 리더를 선출 (동시에 여러 리더가 존재할 수 있음)
    - 초대받은 프로세스도 리더라면 두 그룹을 병합
- Ring Algorithm
    - 모든 노드는 링 형태로 연결됨
    - 각 노드는 링의 토폴로지 정보를 알고 있음
    - 장애를 감지한 노드가 링 전체에 메시지를 순회시켜 활성 노드 목록을 가져오고 순위가 가장 높은 노드를 리더로 선출