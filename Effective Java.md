...
- **상속을 고려해 설계하고 문서화하라. 그러지 않았다면 상속을 금지하라**
    - 뭘 문서화하라는것이지?
        - 재정의할 수 있는 메소드들이 내부적으로 어떻게 이용되는지
        - 또는 해당 메소드가 어떤 일을 수행하는지
    - 하지만 내부 동작을 뜯어본다는 것 = 캡슐화를 해친다는 것 의 단점도 있음
    - 클래스를 상속용으로 설계하는 것은 오버라이드, 생성자 호출 순서 등 제약이 많고 상당한 노력이 필요함
- **추상 클래스보다는 인터페이스를 우선하라**
    - 인터페이스의 구현은 여러 구현이 가능함 → 믹스인 정의에 안성맞춤
    - 인터페이스는 여러 개의 인터페이스를 상속받을 수도 있음
- **인터페이스는 구현하는 쪽을 생각해 설계하라**
    - 인터페이스에 메소드를 추가하게 되면 구현체에서 컴파일 에러가 발생함
    - 이를 해결하기 위해 인터페이스가 디폴트 메소드를 선언할 수 있도록 수정되었음
